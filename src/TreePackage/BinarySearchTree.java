             ...
             implements SearchTreeInterface<T>, java.io.Serializable
{
  ...
  // 27.09
   ...
   public String toString() {
      BinaryNodeInterface root=this.getRootNode();
      StringBuilder builder=new StringBuilder();
		printTree(root, 0, builder, Branch.ROOT);
		return builder.toString();
	}
 
	private static void printTree(BinaryNodeInterface root, int indent, StringBuilder builder, Branch branch) {
      if (root.getData() != null) {
         builder.append(genIndent(indent));
         if (branch==Branch.ROOT) builder.append("+--");
         else builder.append((branch == Branch.LEFT) ? "L--" : "R--");
         builder.append(root.getData().toString());
         builder.append("\n");
      }
      if (root.getLeftChild() != null) {
         printTree(root.getLeftChild(), indent+1, builder, Branch.LEFT);        
      } 
      if (root.getRightChild() != null) {
         printTree(root.getRightChild(), indent+1, builder, Branch.RIGHT);
      } 
   }
